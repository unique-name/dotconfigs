...
sudo apt-get update
sudo apt autoremove
sudo apt-get -y install ssh meld ubuntu-restricted-extras texinfo python-pip colordiff ntfs-config
...
sudo apt-get -y install module-init-tools realpath openconnect repo device-tree-compiler python-dev icu-devtools autotools-dev expect ccache
